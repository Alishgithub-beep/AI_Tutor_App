...
def get_llm():
    try:
        return ChatGroq(
            temperature=0.7,
            model_name="llama-3.1-8b-instant",  # or llama3-8b-8192
...
def _create_tutoring_prompt(subject,level,question,learning_style,background,language):
    ...
    """
    #Build the prompt with all necessary context and instructions 
    prompt = f""" 
    ...
    INSTRUCTIONS:
    1. Provide a clear, educational explanation that directly
    addresses the question
    2. Tailor your explanation to a {background} student at {level} level
    3. Use {language} as the primary language
    4. Format your response with appropriate markdown for readbility
...
def _validate_quiz_data(quiz_data):
    ...
    for question in quiz_data:
        ...
        if not all(key in question for key in ["question","options","correct_answer"]):
            raise ValueError("Each quiz item must contain question, options, correct_answer.")
    ...
    for i, question in enumerate(quiz_data,1):
        ...
        for j, option in enumerate(question["options"]):
            ...
            html += f"""
                    ...
                    onclick="selectOption({i},{j},{is_correct})">
    ...
    html +="""
        ...
        <script>
            function selectOption(questionNum,optionNum,isCorrect){
               ...
               //scroll to answer
               setTimeout(() => {answerDiv.scrollIntoView({behavior:'smooth',block:'nearest'}); 
               },100);

               //Add animation for attention answerDiv.animate([{
